...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ status, useGroq, useBytez }) => {
  ...
  const steps = useMemo(() => {
  const baseSteps: Array<{ id: string; label: string; icon: React.ComponentType<any> }> = [
    ...
    <div className="w-full max-w-2xl mx-auto py-16 px-4 flex flex-col items-center justify-center animate-fade-in">
      ...
      <div className="w-full space-y-4">
        {steps.map((step, index) => {
          ...
          const isCompleted = currentStepIndex > stepIndex || status === AnalysisStatus.COMPLETE;
              ...
              <div className="flex-1">
                ...
                {isActive && (
                  <div className="h-1.5 w-full bg-indigo-100 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-600 rounded-full w-1/2 animate-[wiggle_1s_ease-in-out_infinite]"></div>
                  </div>
                )}
              </div>
            </div>
